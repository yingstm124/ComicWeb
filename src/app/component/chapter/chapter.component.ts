...
export class ChapterComponent implements OnInit {
  ...
  refreshcomic() {
    ...
    this.comicService.getComicByName(this.comic.name).subscribe((val) => {
      this.comics = val.map((e) => {
        (this.comic_votelist = e.payload.doc.data()['vote_list']), (this.comic_votenum = this.comic_votelist.length);
        (this.comic_sublist = e.payload.doc.data()['sub_list']), (this.comic_subnum = this.comic_sublist.length);
  ...
  getRecommentComic() {
    this.comicService.RecommentComic(this.comic.genre).subscribe((val) => {
      this.recommend = val.map((e) => {
        return {
          id: e.payload.doc.id,
          ...e.payload.doc.data(),
        } as Comic;
